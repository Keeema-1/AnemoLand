...
base_path = '../data/anemoland/loot_table/item/weapon/'

def makedir(path):
	if not os.path.isdir(os.path.dirname(path)):
		os.makedirs(os.path.dirname(path))

attack_skills = data["attack_skills"]
...
base_path = '../data/anemoland/function/command/give/weapon/'
...
for weapon_data in data["weapons"]:
	for level in weapon_data["levels"]:
		...
		makedir(path)
		output.append("loot give @s loot anemoland:item/weapon/" + weapon_data["name"] + '/' + str(level))
		with open(path, 'w', encoding='utf-8') as f:
			f.writelines(output)
