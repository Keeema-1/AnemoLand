	...
	for field_id, field_data in map_data["fields"].items():
		...
		if field_data["type"] == "field" or field_data["type"] == "village":
			...
			if not skip_battle_process and not ("special_field" in field_data and field_data["special_field"] == "arena"):
				...
				output.append('execute unless data storage temp:_ data.event{with_npc1:1b} if data storage anemoland:progress data.' + map_name + '.field.' + field_id + '{cleared_once:1b} at @p run function ' + namespace_core + ':sys/player/area/common/field/summon_npc\n')
				output.append('execute if data storage temp:_ data.event{with_npc1:1b} at @p run function ' + namespace_core + ':sys/player/area/common/field/summon_npc\n')
