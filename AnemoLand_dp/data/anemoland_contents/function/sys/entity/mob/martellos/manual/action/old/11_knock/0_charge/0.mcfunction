...
execute unless score #hostile_target.is_player temp matches 1 rotated ~ 0 positioned ^ ^ ^7 facing entity @e[type=#rpg:mob_core,tag=hostile_target,distance=..32,limit=1] feet rotated ~ 0 run function anemoland:sys/entity/common/rot2vec/0
...
execute if score @s action_time matches 45.. run function rpg:sys/entity/mob/martellos/tick/action/11_knock/1_knock/start
execute if entity @s[tag=angry] if score @s action_time matches 35.. run function rpg:sys/entity/mob/martellos/tick/action/11_knock/1_knock/start
