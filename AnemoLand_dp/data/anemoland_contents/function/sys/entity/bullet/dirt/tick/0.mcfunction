...
# 水平移動
    ...
    execute if entity @s[tag=!bullet.dirt.large] store result storage temp:_ data.horizontal_speed.speed float 0.04 run scoreboard players get @s action_way
    execute if entity @s[tag=bullet.dirt.large] store result storage temp:_ data.horizontal_speed.speed float 0.03 run scoreboard players get @s action_way
...
# ヒット処理
    # default
        ...
        # ペットの弾：敵にヒット
            execute if entity @s[tag=pet_bullet] at @s if block ~ ~ ~ #anemoland:space positioned ~-1 ~-1 ~-1 if entity @e[type=#anemoland:mob_core,tag=mob_root,tag=enemy,tag=!damaged,dx=1,dy=1,dz=1,limit=1] positioned ~1 ~1 ~1 run return run function anemoland_contents:sys/entity/bullet/dirt/tick/hit
    # 地面にヒット
        execute at @s unless block ~ ~0.5 ~ #anemoland:space positioned ^ ^1 ^ run return run function anemoland_contents:sys/entity/bullet/dirt/tick/hit
    # 5秒経過でヒット処理
        execute if score @s action_time matches 100.. run return run function anemoland_contents:sys/entity/bullet/dirt/tick/hit
