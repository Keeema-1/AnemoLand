...
# 水平移動
    ...
    execute store result storage temp:_ data.horizontal_speed.speed float 0.04 run scoreboard players get @s action_way
    function anemoland_contents:sys/entity/bullet/water/tick/horizontal_motion with storage temp:_ data.horizontal_speed

# ヒット処理
    # 敵の弾：味方にヒット
        execute if entity @s[tag=!pet_bullet] at @s if block ~ ~ ~ #anemoland:space positioned ~-1 ~-1 ~-1 if entity @a[dx=1,dy=1,dz=1,gamemode=adventure] positioned ~1 ~1 ~1 run return run function anemoland_contents:sys/entity/bullet/water/tick/hit
    ...
    # ペットの弾：敵にヒット
        execute if entity @s[tag=pet_bullet] at @s if block ~ ~ ~ #anemoland:space positioned ~-1 ~-1 ~-1 if entity @e[type=#anemoland:mob_core,tag=mob_root,tag=enemy,tag=!damaged,dx=1,dy=1,dz=1,limit=1] positioned ~1 ~1 ~1 run return run function anemoland_contents:sys/entity/bullet/water/tick/hit
    # 地面にヒット
