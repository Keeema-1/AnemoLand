...
# 設定に応じてカメラを切り替え
    ...
    execute if entity @s[tag=movie.near] run spectate @e[type=armor_stand,tag=camera.near,tag=movie.muscleroom,limit=1]
    execute if entity @s[tag=!movie.near] run spectate @e[type=armor_stand,tag=camera.far,tag=movie.muscleroom,limit=1]
# フェードアウト・フェードイン演出
