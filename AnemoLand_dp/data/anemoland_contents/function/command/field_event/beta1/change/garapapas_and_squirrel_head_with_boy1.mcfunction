$data modify storage anemoland:progress data.beta1.field.$(field_id).event.event_id set value "garapapas_and_squirrel_head_with_boy1"
$data modify storage anemoland:progress data.beta1.field.$(field_id).event.waves set value [{"id": "garapapas_and_squirrel_head", "level": "$(level)"}]
