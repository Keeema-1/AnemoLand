$data modify storage anemoland:progress data.beta1.field.$(field_id).event.event_id set value "many_walking_mushrooms"
$data modify storage anemoland:progress data.beta1.field.$(field_id).event.waves set value [{"id": "many_walking_mushrooms", "level": "$(level)"}]
