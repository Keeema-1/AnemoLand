$data modify storage anemoland:progress data.beta1.field.$(field_id).event.event_id set value "garapases_penguin"
$data modify storage anemoland:progress data.beta1.field.$(field_id).event.waves set value [{"id": "garapases_penguin", "level": "$(level)"}]
