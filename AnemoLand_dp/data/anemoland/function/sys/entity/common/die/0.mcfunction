# execute if score @s level matches 15.. unless data storage anemoland:progress data.armor.iron{unlocked:1b} run function anemoland:command/progress/unlock/armor/iron
...
execute as @e[type=#anemoland:mob_core,tag=mob,distance=..32] if score @s entity_id = #entity_id temp run kill @s
