...
execute if entity @s[tag=enemy] unless score #hostile_target.exist temp matches 1 as @e[type=#anemoland:mob_core,tag=pet,distance=..32,sort=nearest,limit=1] run function anemoland:sys/entity/common/target/tick/mob
execute if entity @s[tag=pet] as @e[type=#anemoland:mob_core,tag=enemy,distance=..32,sort=nearest,limit=1] run function anemoland:sys/entity/common/target/tick/mob
