execute unless score @s sneak_time matches 1.. unless entity @a[tag=!player_check] run return run title @s actionbar [{"translate": "anemoland.general.sneak_to_enter","color": "yellow"}]
...
execute unless score @s sneak_time matches 1.. run return run title @s actionbar [{"text": "スニークでフィールドに移動","color": "yellow"},{"text": " (","color": "yellow"},{"translate":"anemoland.general.num_of_participants","color": "yellow","with":[{"score":{"name":"#members","objective":"temp"}}]},{"text": ")","color": "yellow"}]
